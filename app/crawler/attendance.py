...
class AttendanceCrawler(BaseCrawler):
    ...
    async def fetch_team_attendance(self, ot_date: str = None, dept_name: str = None):
        ...
        mapped_rows = map_attendance_keys(daily_results)
    ...
    async def get_attendance_personal_monthly(self, year, month):
        """
        year: '2026' 형식
        month: 1~12
        """
        try:
            attendance_url = f"{LOGIN_INFO['domain']}/AttendR2/AttendReportPersonMonthByDate_iframe?select_year={year}&select_month={int(month)}&LookupUserID={self.username}"
            await self.page.goto(attendance_url, wait_until="domcontentloaded")

            # 3. 테이블 접근 방법 개선 (table.nth(2) 대신 확실한 키워드 기반 접근)
            # "일자"라는 텍스트를 가진 table_title 클래스의 td가 포함된 테이블을 찾습니다.
            target_table_locator = self.page.locator('table:has(td.table_title:has-text("일자"))').first

            # 해당 테이블이 화면에 렌더링 될 때까지 대기
            await target_table_locator.wait_for(state="attached", timeout=15000)

            # 4. 안전하게 HTML 추출
            table_html = await target_table_locator.inner_html()

            return BeautifulSoup(f"<table>{table_html}</table>", 'html.parser')
    ...
    def expand_second_header_row(header_cells, rowspan1):
        ...
        for rs in rowspan1:
            if rs == 2:
                ...
                # h2 셀 자체가 colspan을 가졌을 경우의 처리
                if skip > 0:
                    names.append(names[-1])
                    skip -= 1
                # 핵심: 인덱스 에러 방지 (header_cells에 남은 셀이 있을 때만 접근)
                    ...
                    if colspan > 1:
                        skip = colspan - 1
                    idx += 1
                # HTML 구조가 누락되어 더 이상 꺼낼 셀이 없으면 빈 문자열로 안전하게 땜빵
                else:
                    names.append('')
